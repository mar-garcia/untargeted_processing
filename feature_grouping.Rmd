...
for(i in 1:length(ms2list)){
  ms2int = c(ms2int, max(ms2list[[i]]@spectrum[,2]))
}

# seed for FG nubmer
n <- 0

for(l in 1:3#length(ms2list)
){
  i = order(-ms2int)[l]
      ...
      for(j in 1:length(idx)){
        if(is.na(features$MS2[idx[j]])){
          if(length(add$assignation[abs((
            features$mzmed[idx[j]] -ms2list[[i]]@precursor) - 
            add$mzdiff) < 0.001])>0){
            features$MS2[idx[j]] <- 
              paste(round(ms2list[[i]]@precursor, 4),
                    add$assignation[abs((
                      features$mzmed[idx[j]] -ms2list[[i]]@precursor) - 
                        add$mzdiff) < 0.001])
          } else {
            features$MS2[idx[j]] <- paste(
              round(ms2list[[i]]@precursor, 4), "[frag]*")
          }
        } else {
          if(length(add$assignation[abs((
            features$mzmed[idx[j]] -ms2list[[i]]@precursor) - 
            ...
            features$MS2[idx[j]] <- paste(
              features$MS2[idx[j]], 
              paste(round(ms2list[[i]]@precursor, 4), "[frag]*"), 
              sep = "; ")
          }
        }
      }
...
rm(chr1, chr2, features.i, tmp, xdata_n, i, idx, j, rtr)

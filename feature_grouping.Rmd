...
Before starting with this, since probably there is redundant MS2 spectra, that is >1 MS2 spectra for the same precursor ion I would like to generate consensus spectra from the MS2 spectra that derive from the same precursor.

```{r merge-spectra, eval=FALSE}
ptable <- data.frame(
  "FT" = rownames(features),
  "mz" = features$mzmed,
  "RT" = features$rtmed
)
featlist2 <- mergeMS2spectra(ms2list,
                             peaktable = ptable,
                             exclude_unmatched = FALSE)
rm(ms2list, ptable)
save.image("data/RData/featlist2.RData")
```


Now, I'm going to start doing the feature grouping following a series of criteria.

```{r ms2-relations}
load("data/RData/featlist2.RData")
rm(ms2list)
...
for(z in 1:nrow(features)
  ...
  if(is.na(features$FG[rownames(features) == ft])){ 
    ...
    idx <- which(rownames(features) %in% rownames(features.i))
    if(any(!is.na(features.i$FG[idx]))){
      if(length(unique(features$FG[idx][!is.na(
        features$FG[idx])])) == 1){
        features$FG[idx] <- unique(features.i$FG[idx][!is.na(
          features.i$FG[idx])])
      } else {
